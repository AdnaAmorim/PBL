...
- [Requisitos Principais](#requisitos-principais)
  ...
  - [Driver Assembly ARM](#driver-assembly-arm)
  - [Aplicacao C](#aplicacao-c)
- [Configuracao do Hardware — Quartus e Platform Designer](#configuracao-do-hardware--quartus-e-platform-designer)
  - [Mapa de Registradores PIO](#mapa-de-registradores-pio)
- [Driver — Implementacao em Assembly](#driver--implementacao-em-assembly)
  - [rotinas.s — Ciclo de Vida da Conexao](#rotinass--ciclo-de-vida-da-conexao)
  - [instrucoes.s — Protocolo MMIO](#instrucoess--protocolo-mmio)
  - [Conjunto de Instrucoes (ISA)](#conjunto-de-instrucoes-isa)
  - [Codigos de Erro](#codigos-de-erro)
- [driver.h — Interface Publica da API](#driverh--interface-publica-da-api)
- [Aplicacao C — main.c](#aplicacao-c--mainc)
  - [Fluxo de Execucao](#fluxo-de-execucao)
  - [Metricas de Desempenho](#metricas-de-desempenho)
- [Build e Execucao](#build-e-execucao)
- [Co-processador ELM — Resumo](#co-processador-elm--resumo)
...
| Ciclo de   |  |  Protocolo MMIO   |
| vida       |  |  Handshake        |
| /dev/mem   |  |  Opcodes · polling|
+------------+  +-------------------+
             |
             v
+------------------------------------------+
|  Lightweight HPS-to-FPGA Bridge          |
|  Endereco fisico: 0xFF200000             |
|  PIO_DATA_IN  [+0x00]                    |
|  PIO_DATA_OUT [+0x10]                    |
|  PIO_CTRL     [+0x20]                    |
+------------------------------------------+
             ...
             v
+------------------------------------------+
|  Co-processador ELM (FPGA — Verilog)     |
|  FSM · MAC · tanh PWL · argmax           |
+------------------------------------------+
```
...
|
+-- arquivos/
|   +-- W_in_q.hex / .mif / .txt    <- Pesos da camada oculta (Q4.12, 100.352 valores)
|   +-- b_q.hex  / .mif / .txt      <- Bias da camada oculta  (Q4.12, 128 valores)
|   +-- beta_q.hex / .mif / .txt    <- Pesos da camada saida  (Q4.12, 1.280 valores)
|   +-- mnist_png/                  <- Dataset MNIST por digito (0-9)
|
+-- coprocessador/
|   +-- CoProcessor.v               <- Unidade de controle (FSM 4 estados)
|   +-- neural_unit.v               <- Orquestrador de inferencia (FSM 5 estados)
|   +-- first_layer.v               <- Camada oculta 784->128 (2 FSMs concorrentes)
|   +-- second_layer.v              <- Camada de saida 128->10
|   +-- mac_first_layer.v           <- MAC com conversao uint8 -> Q4.12
|   +-- mac_second_layer.v          <- MAC Q4.12 x Q4.12
|   +-- tanh_pwl_q4_12.v            <- tanh por 4 segmentos lineares (combinacional)
|   +-- lsu_controller.v            <- Load/Store Unit (wrapper altsyncram M10K)
|   +-- argmax_iterativo.v          <- Argmax em 20 ciclos (FSM 3 estados)
|   +-- reg_bank128.v               <- 128 registradores Q4.12
|   +-- reg_bank10.v                <- 10 registradores Q4.12
|   +-- display_resultado.v         <- Decodificador 4b -> 7 segmentos (HEX7)
|   +-- ghrd_top.v                  <- Top-level SoC (HPS + FPGA + PIOs)
|   +-- soc_system.qpf              <- Projeto Quartus Prime
|   +-- soc_system.qsf              <- Atribuicoes de pinos e configuracoes
|   +-- soc_system.qsys             <- Platform Designer (PIOs + HPS + bridges)
|   +-- soc_system_timing.sdc       <- Constraints de timing SDC
|   +-- hps_0.h                     <- Header C gerado pelo Platform Designer
|
+-- driver/
    +-- main.c                      <- Benchmark: 1.000 inferencias + metricas
    +-- driver.h                    <- Interface publica da API (declaracoes C)
    +-- instrucoes.s                <- Protocolo MMIO, handshake, opcodes (Assembly ARMv7)
    +-- rotinas.s                   <- Ciclo de vida da conexao FPGA (Assembly ARMv7)
    +-- Makefile                    <- gcc + -lrt, target ARM Linux (DE1-SoC)
